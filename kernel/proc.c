...
freeproc(struct proc *p)
{
  ...
  p->xstate = 0;
  p->state = UNUSED;
  p->rtime = 0; //reset runtime
  p->waittime = 0; //reset to default waittime
  p->priority=60; //reset to default priority
}
...
int getDynamPriority(struct proc *p) {
  ...
  if (p->rtime >0 || p->waittime > 0) {
    niceness = p->waittime*10/(p->rtime + p->waittime);
  } 
  else {
    niceness=5; //new process
  }
  min_a=p->priority-niceness+5;
  if (min_a>100) {
    min_a=100;  //min(sp-niceness+5,100)
  }
  max_a=min_a;
  if (max_a<0) {
    max_a=0;  //max(min_a,0)
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE) {
        // Switch to min_proc process.  It is the process's job
  ...
  // }
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        if (!process)
        {
          process = p;  //if no proc add proc
          continue;
        }
        else if (getDynamPriority(process) > getDynamPriority(p))
        {
        ...
        {
          if (process->frequency > p->frequency)
          {
            release(&process->lock);
            process = p;
            continue;
          }
          else if (process->frequency == p->frequency)
          {
            if (process->addedtime < p->addedtime)
            {
              release(&process->lock);
              process = p;
              continue;
...
int
waitx(uint64 addr, uint* waittime, uint* rtime)
{
  struct proc *np;
  int havekids, pid;
  struct proc *p = myproc();
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
#ifdef RR
    printf("\n PID: %d state: %s name: %s", p->pid, state, p->name);
#endif    
